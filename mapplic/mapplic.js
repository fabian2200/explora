...
(function($) {
	...
	var Mapplic = function() {
		...
		self.init = async function(el, params) {
			...
			self.y = 0;
			self.scale = 1;
			self.el = el.addClass('mapplic-element mapplic-loading').addClass(self.o.skin).height("100vh");
			...
			if (typeof self.o.source === 'string') {
				cargarDepartamentos(self.o.source).then(function(data) {
					processData(data);
				...
				cargarDepartamentos("departamentos.json").then(function(data) {
					processData(data);
					self.el.removeClass('mapplic-loading');
				}).fail(function() {
					console.error("Error al cargar los datos.");
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				this.el2 = $('<div id="opciones_multimedia"></div>').addClass('mapplic-tooltip-2').addClass('multimedia-div');
				this.video = $('<div onclick="verVideo(this);"></div>').addClass('mapplic-tooltip-video').appendTo(this.el2);
				this.imagenes  =  $('<div onclick="verImagenes(this);"></div>').addClass('mapplic-tooltip-imagenes').appendTo(this.el2);
				this.play  =  $('<div></div>').addClass('mapplic-tooltip-play').appendTo(this.el2);
				...
				this.el3 = $('<br><div></div>');
				this.click = $('<a onclick="verMas(this);">Ver mas <i class="fa fa-search" aria-hidden="true"></i></a>').addClass('mapplic-tooltip-link').appendTo(this.el3);
				this.el3.appendTo(this.el);
				// Append
			...
			this.set = function(location) {
				if (location) {
					...
					if(location.video) this.video.attr('link-video', location.video).show();
					else this.video.hide();

					this.click.attr('id-departamento', location.id_departamento);

					if(location.imagenes || location.video || location.audio){
						document.getElementById("opciones_multimedia").style.display = "flex";
					}else{	
						document.getElementById("opciones_multimedia").style.display = "none";
					}
		...
		var leerJson = function(departamento) {
			return new Promise((resolve, reject) => {
				fetch('departamentos.json')
					.then(response => response.json())
					.then(data => {
						const departamentoFiltrado = data.find(item => item.departamento === departamento);
						if (departamentoFiltrado) {
							resolve(departamentoFiltrado);
						} else {
							reject(new Error(`No se encontró el departamento ${departamento}`));
						}
					})
					.catch(error => {
						reject(error);
		...
		var mapearOpcionesMenu = function(opciones) {
			var sip = document.getElementById("sitios_interes_panel");

			var div = "";
			opciones.sitios_de_interes.forEach(element => {
				div += "<li data-toggle='modal' data-target='#modal_sitios' class='li_opciones' style='text-align: left; margin-top: 10px'><i class='fa-solid fa-signs-post'></i> "+element+"</li>";
			});

			sip.innerHTML = "";
			sip.innerHTML = div;

			var fp = document.getElementById("festividades_panel");

			var div2 = "";
			opciones.festividades.forEach(element => {
				div2 += "<li class='li_opciones' style='text-align: left; margin-top: 10px'><i class='fa-regular fa-calendar-plus'></i> "+element+"</li>";
			});

			fp.innerHTML = "";
			fp.innerHTML = div2;

			var ep = document.getElementById("etnias_panel");

			var div3 = "";
			opciones.etnias.forEach(element => {
				div3 += "<li class='li_opciones' style='text-align: left; margin-top: 10px'><i class='fa-solid fa-people-group'></i> "+element+"</li>";
			});
